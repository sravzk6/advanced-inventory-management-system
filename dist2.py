...
def visualize_graph(G, pos, edge_labels):
    ...
    for edge, weight in edge_labels.items():
        ...
        dx = (x2 - x1) * 0.1
        dy = (y2 - y1) * 0.1
        plt.text(x + dx, y + dy, str(weight), fontsize=10, ha='center', va='center')
...
def main():
    ...
    try:
        bfs_tree = nx.bfs_tree(G, source=source_node)
        plt.figure()
        nx.draw(bfs_tree, with_labels=True, font_weight='bold', arrows=True)
        plt.title('Directed BFS Tree')
        plt.show()

        dfs_tree = nx.dfs_tree(G, source=source_node)
        plt.figure()
        nx.draw(dfs_tree, with_labels=True, font_weight='bold', arrows=True)
